...
from utils import PINODatasetSingle, PINODatasetMult, setup_seed, generate_input, count_params, to_device
    ...
    for i in range(len(dataloader)):
        ...
        if f_weight>0:
            ...
            if not if_temporal:  # DarcyFlow
                ...
                input = generate_input(a,grid)
            pred = model(input)
            ic_loss, f_loss = pde_loss(pred, a, u, train_args)
        else:
...
@torch.no_grad()
def test_loop(dataloader, model,if_temporal, device, metric_names=['MSE', 'RMSE', 'bRMSE', 'L2RE', 'L1RE', 'MaxError']):
    model.eval()
    # initial result dict
    res_dict = {}
...
def main(args):
    ...
    if not args["if_training"]:
        ...
        # TODO: test code
        print("start testing...")
        res = test_loop(val_loader, model, if_temporal, device)
        print(res)
        print("Done")
        return
