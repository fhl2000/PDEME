...
def MSE(pred, target):
    ...
    """
    assert pred.shape == target.shape
    res = torch.mean( (pred - target).pow(2), dim=0 )
    return res
...
def L2RE(pred, target):
    ...
    assert pred.shape == target.shape
    err = pred - target
    res = torch.mean(err.pow(2), dim=0) / torch.mean(target.pow(2), dim=0)
...
def L1RE(pred, target):
    ...
    assert pred.shape == target.shape
    err = pred - target
    res = torch.mean(torch.abs(err), dim=0) / torch.mean(torch.abs(target), dim=0)
    return res
