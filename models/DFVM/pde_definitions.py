...
def pde_adv1d(x, model, beta):
    x, xL, xR = x
    y = model(x)
    dy_t = dde.grad.jacobian(y, x, i=0, j=1)
    yL = model(xL)
    yR = model(xR)
    res = dy_t*0.0002 + beta*(yR - yL)
    return res.reshape(-1, 1)

def pde_diffusion_reaction_1d(x, model, nu, rho):
    x, xL, xR = x
    y = model(x)
    dy_t  = dde.grad.jacobian(y, x, i=0, j=1)
    yL = model(xL)
    yR = model(xR)
    dyR_x = dde.grad.jacobian(yR, xR, i=0, j=0)
    dyL_x = dde.grad.jacobian(yL, xL, i=0, j=0)
    res = (dy_t - rho*y*(1.-y))*0.0002 - nu*(dyR_x - dyL_x)
    return res.reshape(-1, 1)
...
def pde_Allen_Cahn(x, model, c1, c2):
    x, xL, xR = x
    y = model(x)
    ...
    dyL_x = dde.grad.jacobian(yL, xL, i=0, j=0)
    phi = y**3 - y
    res = (u_t + c2*phi)*0.0002 - c1*(dyR_x - dyL_x)
    return res.reshape(-1, 1)

def pde_Cahn_Hilliard(x, model, gamma1, gamma2):
    x, xL, xR = x
    y = model(x)
    u_t = dde.grad.jacobian(y, x, i=0, j=1)
    yL = model(xL)
    yR = model(xR)
    dyR_x = dde.grad.hessian(yR, xR, i=0, j=0)
    dyL_x = dde.grad.hessian(yL, xL, i=0, j=0)
    phi1 = yL**3 - yL
    phi2 = yR**3 - yR
    p1 = gamma2*phi1 - gamma1*dyL_x
    p2 = gamma2*phi2 - gamma1*dyR_x
    p1_x = dde.grad.jacobian(p1, xL, i=0, j=0)
    p2_x = dde.grad.jacobian(p2, xR, i=0, j=0)
    res = u_t*0.0002 + (p1_x - p2_x)
    return res.reshape(-1, 1)

def pde_CFD1d(x, model, gamma):
    x, xL, xR = x
    y = model(x)
    yL = model(xL)
    yR = model(xR)
    ...
    u = y[..., 1].unsqueeze(1)  # v
    p = y[..., 2].unsqueeze(1)  # p
    hL = yL[..., 0].unsqueeze(1)  # rho
    uL = yL[..., 1].unsqueeze(1)  # v
    pL = yL[..., 2].unsqueeze(1)  # p
    hR = yR[..., 0].unsqueeze(1)  # rho
    uR = yR[..., 1].unsqueeze(1)  # v
    pR = yR[..., 2].unsqueeze(1)  # p

    E = p/(gamma - 1.) + 0.5 * h * u**2
    E = E.unsqueeze(1)
    EL = pL/(gamma - 1.) + 0.5 * hL * uL**2
    EL = EL.unsqueeze(1)
    ER = pR/(gamma - 1.) + 0.5 * hR * uR**2
    ER = ER.unsqueeze(1)
    ...
    uR_x = dde.grad.jacobian(yR, xR, i=1, j=0)
    uL_x = dde.grad.jacobian(yL, xL, i=1, j=0)
    E_t = dde.grad.jacobian(E, x, i=0, j=1)

    eq1 = h_t*0.0002 + hR*uR - hL*uL
    eq2 = h * (u_t + u * u_x) * 0.0002 + (pR-pL) - (eta*4/3 + zeta)*(uR_x-uL_x)
    eq3 = E_t*0.0002 + (ER+pR)*uR_x - (EL+pL)*uL_x
    res = eq1 + eq2 + eq3
    return res.reshape(-1, 1)
