...
from utils import PDE, MPNNDatasetSingle, MPNNDatasetMult, GraphCreator
from utils import setup_seed, to_PDEBench_format
...
def get_model(args, pde: PDE):
    model = MPNN(pde, time_window=args["time_window"], hidden_features=args["model"]["hidden_features"],
                hidden_layer=args["model"]["hidden_layer"], eq_variables=args["variables"])
    return model
...
def val_loop(dataloader, model, loss_fn, device, graph_creator):
    ...
    for u, x, variables in dataloader:
        ...
        target = torch.cat((target, graph.y), dim=-1)
        with torch.no_grad():
            output = model(graph)
        pred = torch.cat((pred, output), dim=-1)

        for step in range(graph_creator.tw+1, graph_creator.nt - graph_creator.tw + 1, graph_creator.tw):
...
def test_loop(dataloader, model, device, graph_creator, metric_names=['MSE', 'RMSE', 'bRMSE', 'L2RE', 'L1RE', 'MaxError']):
    model.eval()
    res_dict = {}
