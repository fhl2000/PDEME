...
def get_model(spatial_dim, if_temporal, args):
    ...
    if spatial_dim == 1:
        model = DeepONetCartesianProd1D(size=model_args["size"],
        ...
        model = DeepONetCartesianProd2D(size=model_args["size"],
                    in_channel_branch=model_args["in_channels"]*initial_step,
                      query_dim= model_args["query_dim"],
                      out_channel=model_args["out_channels"],
                      activation=model_args["act"],
...
@torch.no_grad()
def test_loop(dataloader, model, initial_step, if_temporal, device, train_args, metric_names=['MSE', 'RMSE', 'bRMSE', 'L2RE', 'L1RE', 'MaxError']):
    model.eval()
    # initial result dict
    res_dict = {}
...
def main(args):
    ...
    if not args["if_training"]:
        ...
        # TODO: test code
        print("start testing...")
        res = test_loop(val_loader, model, args["initial_step"], if_temporal, device, args["train"])
        print(res)
        print("Done")
        return
